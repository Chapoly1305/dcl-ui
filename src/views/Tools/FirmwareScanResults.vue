...
import { DISPLAY_STAGES, stageSeverity as sharedStageSeverity, stageStatusBadgeLabel } from '@/utils/pipelineDisplay';
...
export default {
    ...
    methods: {
        ...
        stageSeverity(value) {
            return sharedStageSeverity(value);
        },
        stageStatusLabel(value) {
            return stageStatusBadgeLabel(value);
        },
                    ...
                    <TabPanel header="Stages">
                        ...
                        <div v-else>
                            ...
                            <div class="text-xs mb-3 flex flex-wrap align-items-center gap-3">
                                <span class="cl-tally"
                                    ><span class="cl-dot cl-dot-success"></span> Passed <strong>{{ stageStatusCounts.passed }}</strong></span
                                >
                                <span class="cl-tally"
                                    ><span class="cl-dot cl-dot-secondary"></span> Skipped <strong>{{ stageStatusCounts.skipped }}</strong></span
                                >
                                <span class="cl-tally"
                                    ><span class="cl-dot cl-dot-warning"></span> Pending <strong>{{ stageStatusCounts.pending }}</strong></span
                                >
                                <span class="cl-tally"
                                    ><span class="cl-dot cl-dot-info"></span> Needs review <strong>{{ stageStatusCounts.review }}</strong></span
                                >
                                <span class="cl-tally"
                                    ><span class="cl-dot cl-dot-danger"></span> Issues <strong>{{ stageStatusCounts.issue }}</strong></span
                                >
                            </div>
                                        ...
                                        <div v-if="isExpanded('stage-' + stage.name)" class="stage-expanded mt-2">
                                            ...
                                            <div v-if="stageSections(stage.name).length > 0" class="checklist-grid">
                                                <div v-for="sec in stageSections(stage.name)" :key="sec.id" class="checklist-item" :class="'cl-' + checklistStatus(sec.id)">
                                                    ...
                                                    <Button v-if="sectionHasTranscript(sec.id)" label="View AI transcript" icon="pi pi-comments" text size="small" class="cl-transcript-btn mt-1" @click.stop="openTranscriptDialog(sec.id)" />
                                                </div>
                                            </div>
